...
exports.testStkPush = exports.getDarajaLogs = exports.getDarajaStatus = exports.sendSmsBroadcast = exports.getSmsLogs = exports.getCurrentSubscription = exports.registerChamaTenant = exports.getAuditLogs = exports.getPlatformAnalytics = exports.deleteTenant = exports.updateTenantStatus = exports.getTenants = exports.upsertSubscriptionPlan = exports.getSubscriptionPlans = void 0;
...
const upsertSubscriptionPlan = async (req, res) => {
    try {
        ...
        const plan = await prisma_1.prisma.subscriptionPlan.upsert({
            where: { code },
            update: {
                name,
                description,
...
const updateTenantStatus = async (req, res) => {
    try {
        ...
        const previousChama = await prisma_1.prisma.chama.findUnique({ where: { id } });
        if (!previousChama) {
            return res.status(404).json({ error: 'Tenant not found' });
        }
        const updated = await prisma_1.prisma.chama.update({
            where: { id },
            data: { status }
        });
        await (0, auditLogger_1.logAuditEvent)({
...
const getPlatformAnalytics = async (req, res) => {
    try {
        ...
        const previousDateFilter = (previousStartDate && previousEndDate)
            ...
            : {};
        const totalTenants = await prisma_1.prisma.chama.count();
        const activeTenants = await prisma_1.prisma.chama.count({ where: { status: 'ACTIVE' } });
        const trialTenants = await prisma_1.prisma.chama.count({ where: { status: 'TRIAL' } });
        const pendingVerifications = await prisma_1.prisma.chama.count({ where: { status: 'PENDING_APPROVAL' } });
        const totalMembers = await prisma_1.prisma.user.count({ where: { role: 'MEMBER' } });
        // Calculate Monthly Recurring Revenue (MRR) dynamically (we assume subs active during this period)
        const activeSubscriptions = await prisma_1.prisma.chamaSubscription.findMany({
        ...
        const previousSubscriptions = await prisma_1.prisma.chamaSubscription.findMany({
        ...
        const totalTransactions = await prisma_1.prisma.payment.count({ where: dateFilter });
        ...
        const chamas = await prisma_1.prisma.chama.findMany({ include: { ledgers: true } });
        ...
        for (let i = 5; i >= 0; i--) {
            ...
            const activeSubs = await prisma_1.prisma.chamaSubscription.findMany({
        ...
        // Top Chamas Data
        const topChamasRaw = await prisma_1.prisma.chama.findMany({
            include: {
 ...
 */
const registerChamaTenant = async (req, res) => {
    try {
        const { chamaName, registrationNumber, county, meetingFrequency, standardContribution, adminName, adminEmail, adminPhone, adminPassword, selectedPlanCode = 'STARTER' } = req.body;
        ...
        const existingReg = await prisma_1.prisma.chama.findUnique({ where: { registration: registrationNumber } });
        if (existingReg) {
        ...
        }) || await prisma_1.prisma.subscriptionPlan.findUnique({ where: { code: 'STARTER' } });
        if (!plan) {
...
exports.registerChamaTenant = registerChamaTenant;
/**
...
const getCurrentSubscription = async (req, res) => {
    try {
        ...
        let subscription = await prisma_1.prisma.chamaSubscription.findUnique({
            where: { chamaId },
            include: {
                plan: true,
                invoices: {
        ...
        if (!subscription) {
            const defaultPlan = await prisma_1.prisma.subscriptionPlan.findFirst({ where: { code: 'STARTER' } }) || await prisma_1.prisma.subscriptionPlan.findFirst();
            if (defaultPlan) {
                subscription = await prisma_1.prisma.chamaSubscription.create({
                    data: {
                        chamaId,
        ...
        const currentMonthPayments = await prisma_1.prisma.payment.count({
            where: {
                chamaId,
                createdAt: {
...
const getSmsLogs = async (req, res) => {
    try {
        let logs = await prisma_1.prisma.communicationLog.findMany({
            include: {
                chama: {
                    select: { name: true }
                }
        ...
        if (logs.length === 0) {
            const defaultChama = await prisma_1.prisma.chama.findFirst();
            if (defaultChama) {
                await prisma_1.prisma.communicationLog.createMany({
                    data: [
                        {
                            chamaId: defaultChama.id,
                            type: 'SMS',
                ...
                logs = await prisma_1.prisma.communicationLog.findMany({
                    include: {
                        chama: {
                            select: { name: true }
                        }
 ...
 */
const sendSmsBroadcast = async (req, res) => {
    try {
 ...
 */
const getDarajaStatus = async (req, res) => {
    try {
        const totalPayments = await prisma_1.prisma.payment.count();
        const totalVolume = await prisma_1.prisma.payment.aggregate({
            _sum: { amount: true }
        });
        const activeChamasWithPaybill = await prisma_1.prisma.chama.count({
            where: { status: 'ACTIVE' }
        });
        res.json({
            status: 'OPERATIONAL',
...
exports.getDarajaStatus = getDarajaStatus;
/**
 * Super Admin: Get Daraja Transaction Logs
 */
const getDarajaLogs = async (req, res) => {
    try {
        let payments = await prisma_1.prisma.payment.findMany({
            include: {
                chama: {
                    select: { name: true }
                }
...
exports.getDarajaLogs = getDarajaLogs;
/**
...
const testStkPush = async (req, res) => {
    try {
        ...
        if (defaultChama) {
            ...
            // Simulate calling Daraja API
            await prisma_1.prisma.paymentLog.create({
                data: {
                    chamaId: defaultChama.id,
...
exports.testStkPush = testStkPush;
