...
export const getAnalytics = async (req: Request, res: Response) => {
  try {
    ...
    const arrears = await prisma.arrearsRecord.findMany();
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const interestPayments = await prisma.loanRepayment.aggregate({
    ...
    ledgers.forEach(l => {
      if (l.accountType === 'ASSET') totalAssets += l.balance || 0;
      if (l.accountType === 'LIABILITY') totalLiabs += l.balance || 0;
      if (l.accountType === 'EQUITY') totalEquity += l.balance || 0;
      if (l.accountName.toLowerCase().includes('loan')) totalLoans += l.balance || 0;
      if (l.accountName.toLowerCase().includes('deposit') || l.accountName.toLowerCase().includes('saving')) totalDeposits += l.balance || 0;
    });

    // Calculate Net Income from Revenue and Expense accounts
    let revenue = 0, expense = 0;
    ledgers.forEach(l => {
      if (l.accountType === 'REVENUE') revenue += l.balance || 0;
      if (l.accountType === 'EXPENSE') expense += l.balance || 0;
    });
